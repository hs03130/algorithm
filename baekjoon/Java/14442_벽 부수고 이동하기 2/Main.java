...
public class Main {
	...
	static final int X = 0, Y = 1, BROKEN = 2, NOT_VISITED = 0;
	...
	public static void main(String[] args) throws IOException {
		...
		while (!Q.isEmpty() && ans == -1) {
			int[] cur = Q.poll();
			if (cur[X] == boardH - 1 && cur[Y] == boardW - 1) {
				ans = dist[cur[X]][cur[Y]][cur[BROKEN]];
				break;
			}
			...
			for (int dir = 0; dir < 4; dir++) {
				...
				// 벽이고 부술 수 있음
				if (board[nx][ny] == WALL && cur[BROKEN] < breakCnt && dist[nx][ny][cur[BROKEN] + 1] == NOT_VISITED) {
					dist[nx][ny][cur[BROKEN] + 1] = dist[cur[X]][cur[Y]][cur[BROKEN]] + 1;
					Q.offer(new int[] { nx, ny, cur[BROKEN] + 1 });
				}
				// 벽이 아님
				else if (board[nx][ny] != WALL && dist[nx][ny][cur[BROKEN]] == NOT_VISITED) {
